...
// In IMGUI_VERSION_NUM >= 18934: changed IMGUI_TEST_CORE_ITEM_ADD(bb,id) to IMGUI_TEST_CORE_ITEM_ADD(id,bb,item_data);
#define IMGUI_TEST_CORE_ITEM_ADD(_ID,_BB,_ITEM_DATA)      if (g.TestEngineHookItems) ImGuiTestEngineHook_ItemAdd(&g, _ID, _BB, _ITEM_DATA)    // Register item bounding box
#define IMGUI_TEST_CORE_ITEM_INFO(_ID,_LABEL,_FLAGS)      if (g.TestEngineHookItems) ImGuiTestEngineHook_ItemInfo(&g, _ID, _LABEL, _FLAGS)    // Register item label and status flags (optional)
#define IMGUI_TEST_CORE_LOG(_FMT,...)                     if (g.TestEngineHookItems) ImGuiTestEngineHook_Log(&g, _FMT, __VA_ARGS__)           // Custom log entry from user land into test log
#else
#define IMGUI_TEST_CORE_ITEM_ADD(_BB,_ID)                 ((void)0)
#define IMGUI_TEST_CORE_ITEM_INFO(_ID,_LABEL,_FLAGS)      ((void)g)
#endif
